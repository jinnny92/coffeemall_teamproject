...
<table class="table">
  ...
  <c:forEach items="${stocklist}" var="stocklist">
    ...
    <tr>
      ...
      <td class="sellamount">${stocklist.cartCount}</td>
...
<nav aria-label="...">
  <ul class="pagination">
    <li class="page-item">
      <a class="page-link" href="/order/stock?curpage=${pt.curPage <= 1? 1: pt.curPage-1}">&laquo;</a>
    </li>
    
    <c:forEach begin="${pt.beginPageNum}" end="${pt.finishPageNum}" var="page">
       <c:if test="${page == pt.curPage}">
           <li class="page-item active" aria-current="page">
               <a class="page-link" href="/order/stock?curpage=${pt.curPage}">${page}</a>
          </li>
       </c:if>
       
       <c:if test="${page != pt.curPage}">
            <li class="page-item">
            <a class="page-link" href="/order/stock?curpage=${page}">${page}</a></li>
       </c:if>
    ...
    <li class="page-item">
      <a class="page-link" href="/order/stock?curpage=${pt.curPage >= pt.totalPage? pt.totalPage : pt.curPage+1}">&raquo;</a>
    </li>
  </ul>
</nav>
